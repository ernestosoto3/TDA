...
export const editorEntityScopes = pgTable(
...
${editorEntityScopes.teamId}
    ...
    uniqueIndex('editor_entity_scopes_active_sport_unique_idx')
      .on(editorEntityScopes.userRoleId, editorEntityScopes.sportId)
      .where(
    ...
    uniqueIndex('editor_entity_scopes_active_league_unique_idx')
      .on(editorEntityScopes.userRoleId, editorEntityScopes.leagueId)
      .where(
    ...
    uniqueIndex('editor_entity_scopes_active_team_unique_idx')
      .on(editorEntityScopes.userRoleId, editorEntityScopes.teamId)
      .where(
        sql`${editorEntityScopes.teamId} IS NOT NULL AND ${editorEntityScopes.revokedAt} IS NULL`,
      ),

    index('editor_entity_scopes_user_role_id_idx').on(editorEntityScopes.userRoleId),

    index('editor_entity_scopes_sport_id_idx').on(editorEntityScopes.sportId),

    index('editor_entity_scopes_league_id_idx').on(editorEntityScopes.leagueId),

    index('editor_entity_scopes_team_id_idx').on(editorEntityScopes.teamId),

    index('editor_entity_scopes_assigned_by_user_id_idx').on(editorEntityScopes.assignedByUserId),

    index('editor_entity_scopes_approved_by_user_id_idx').on(editorEntityScopes.approvedByUserId),
  ],
);
