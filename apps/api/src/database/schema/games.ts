import {uuid , timestamp , varchar , text , pgTable , check , foreignKey , unique , index} from 'drizzle-orm/pg-core'
...
index('games_league_id_idx').on(
games.leagueId,
),

index('games_home_team_id_idx').on(
games.homeTeamId,
),

index('games_away_team_id_idx').on(
games.awayTeamId,
),

index('games_status_scheduled_start_at_idx').on(
games.status,
games.scheduledStartAt,
),

index('games_league_scheduled_start_at_idx').on(
games.leagueId,
games.scheduledStartAt,
),

index('games_league_away_team_idx').on(
games.leagueId,
games.awayTeamId,
),
],
