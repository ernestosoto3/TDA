...
export const policyVersions = pgTable(
  ...
  (policyVersions) => [
    unique().on(policyVersions.policyType, policyVersions.version),
    ...
    uniqueIndex('policy_versions_current_required_unique_idx')
      .on(policyVersions.policyType)
      .where(
        sql`${policyVersions.status} = 'published' AND ${policyVersions.requiresAcceptance} = true AND ${policyVersions.supersededAt} IS NULL`,
      ),
    ...
    index('policy_versions_created_by_user_idx')
      .on(policyVersions.createdByUserId)
