...
export const moderationEscalations = pgTable(
  'moderation_escalations',
  {
    ...
    escalatedByUserId: uuid('escalated_by_user_id')
      .notNull()
  ...
  (moderationEscalations) => [
    ...
    index('moderation_escalations_report_idx').on(moderationEscalations.reportId),

    index('moderation_escalations_escalated_by_user_idx').on(
      moderationEscalations.escalatedByUserId,
    ),

    index('moderation_escalations_assigned_to_user_idx').on(moderationEscalations.assignedToUserId),

    index('moderation_escalations_resolved_by_user_idx').on(moderationEscalations.resolvedByUserId),
  ],
);
