...
reportedCommentId : uuid('reported_comment_id').references(()=>comments.id,{onDelete:'restrict'}),
reportedMessageId : uuid('reported_message_id').references(()=>messages.id,{onDelete:'restrict'}),
...
index('reports_status_created_idx').on(
reports.status,
reports.createdAt,
),

index('reports_reporter_user_idx').on(
