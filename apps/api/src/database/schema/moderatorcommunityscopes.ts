...
export const moderatorCommunityScopes = pgTable(
  ...
  (moderatorCommunityScopes) => [
    ...
    uniqueIndex('moderator_community_scopes_active_unique_idx')
      .on(moderatorCommunityScopes.userRoleId, moderatorCommunityScopes.communityId)
      .where(sql`${moderatorCommunityScopes.revokedAt} IS NULL`),

    index('moderator_community_scopes_user_role_id_idx').on(moderatorCommunityScopes.userRoleId),

    index('moderator_community_scopes_community_id_idx').on(moderatorCommunityScopes.communityId),
