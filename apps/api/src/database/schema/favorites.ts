...
index('favorites_user_id_idx').on(
favorites.userId,
),

index('favorites_sport_id_idx').on(
favorites.sportId,
).where(sql`${favorites.sportId} IS NOT NULL`),

index('favorites_league_id_idx').on(
favorites.leagueId,
).where(sql`${favorites.leagueId} IS NOT NULL`),

index('favorites_team_id_idx').on(
favorites.teamId,
).where(sql`${favorites.teamId} IS NOT NULL`),

index('favorites_athlete_id_idx').on(
favorites.athleteId,
).where(sql`${favorites.athleteId} IS NOT NULL`),
],
)