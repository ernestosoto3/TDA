...
(${posts.coverMediaUrl} IS NOT NULL AND ${posts.coverMediaType} IS NOT NULL)
    ...
    index('posts_author_user_idx').on(posts.authorUserId),

    index('posts_scheduled_by_user_idx').on(posts.scheduledByUserId),

    index('posts_status_scheduled_for_idx').on(posts.status, posts.scheduledFor),
    ...
    index('posts_status_published_at_idx').on(posts.status, posts.publishedAt.desc()),
