...
export const scores = pgTable(
  ...
  (scores) => [
    check('scores_home_score_non_negative_check', sql`${scores.homeScore} >= 0`),

    check('scores_away_score_non_negative_check', sql`${scores.awayScore} >= 0`),
    ...
    index('scores_winning_team_id_idx').on(scores.winningTeamId),
  ],
);
